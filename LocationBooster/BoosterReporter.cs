...
namespace LocationBudgetBooster
{
    public static class BoosterReporter
    {
        ...
        private static void LogHeartbeat(ReportData data)
        {
            ...
            sb.AppendLine(); // <--- Added gap for readability
            ...
            // CHANGED: Pass 10 spaces instead of 'pad' to prevent double "└─ └─"
            if (data.ErrAlt > 0) failures.Add(("Wrong Altitude", data.ErrAlt, (s, pad) => PrintAlt(s, "          ", data.InstanceHash)));
        ...
        private static void LogFullReport(ReportData data)
        {
            ...
            if (BoosterDiagnostics.GlobalMaxAltitudeSeen > float.MinValue)
            {
                report.AppendLine($"(World Altitude Profile: Min {BoosterDiagnostics.GlobalMinAltitudeSeen:F1}m, Max {BoosterDiagnostics.GlobalMaxAltitudeSeen:F1}m)");
            }
            ...
            // 1. Distance
            steps.Add(new FunnelStep { Name = "DISTANCE FILTER", ConfigInfo = $"(Min: {data.Loc.m_minDistance:F0}, Max: {effectiveMax:F0})", PassedContext = $"Range {data.Loc.m_minDistance:F0}-{effectiveMax:F0}", Input = data.InDist, Failures = data.ErrDist, FailurePrinter = (sb, indent) => PrintDist(sb, indent, data.InstanceHash) });
            // 2. Biome
            steps.Add(new FunnelStep { Name = "BIOME MATCH", ConfigInfo = $"(Required: {data.Loc.m_biome})", PassedContext = $"{data.Loc.m_biome}", Input = data.InBiome, Failures = data.ErrBiome, FailurePrinter = (sb, indent) => PrintDict(sb, $"{indent}    └─ ", BoosterDiagnostics.BiomeFailures, data.LocHash) });
            // 3. Altitude
            steps.Add(new FunnelStep { Name = "ALTITUDE CHECK", ConfigInfo = $"(Min: {data.Loc.m_minAltitude:F0}, Max: {data.Loc.m_maxAltitude:F0})", PassedContext = $"Alt {data.Loc.m_minAltitude:F0} to {data.Loc.m_maxAltitude:F0}", Input = data.InAlt, Failures = data.ErrAlt, FailurePrinter = (sb, indent) => PrintAlt(sb, $"{indent}    ", data.InstanceHash) });
            // 4. Forest
            ...
            for (int i = 0; i < steps.Count; i++)
            {
                ...
                if (i == 0) report.AppendLine($"1. {step.Name} {step.ConfigInfo}");
                else report.AppendLine($"{indent}└─ {i + 1}. {step.Name} {step.ConfigInfo}: {step.Input:N0} points checked");
        ...
        {
            if (BoosterDiagnostics.DistanceTooClose.TryGetValue(hash, out long tooClose) && tooClose > 0) sb.AppendLine($"{prefix}Below Min: {tooClose:N0}");
            if (BoosterDiagnostics.DistanceTooFar.TryGetValue(hash, out long tooFar) && tooFar > 0) sb.AppendLine($"{prefix}Above Max: {tooFar:N0}");
        }
        ...
        private static void PrintAlt(StringBuilder sb, string prefix, int hash)
        {
            ...
            long totalLow = (BoosterDiagnostics.AltitudeTooLow_Standard.ContainsKey(hash) ? BoosterDiagnostics.AltitudeTooLow_Standard[hash].Values.Sum() : 0) +
                            (BoosterDiagnostics.AltitudeTooLow_Anomalous.ContainsKey(hash) ? BoosterDiagnostics.AltitudeTooLow_Anomalous[hash].Values.Sum() : 0) +
                            (BoosterDiagnostics.AltitudeTooLow_Underwater.ContainsKey(hash) ? BoosterDiagnostics.AltitudeTooLow_Underwater[hash].Values.Sum() : 0);

            if (totalLow > 0)
            {
                sb.AppendLine($"{prefix}└─ Too Low: {totalLow:N0}");

                foreach (var biome in allBiomes.OrderBy(b => b.ToString()))
                {
                    bool hasWater = BoosterDiagnostics.AltitudeTooLow_Underwater.TryGetValue(hash, out var watDict) && watDict.ContainsKey(biome);
                    bool hasAnom = BoosterDiagnostics.AltitudeTooLow_Anomalous.TryGetValue(hash, out var anoDict) && anoDict.ContainsKey(biome);
                    bool hasStd = BoosterDiagnostics.AltitudeTooLow_Standard.TryGetValue(hash, out var stdDict) && stdDict.ContainsKey(biome);

                    if (!hasWater && !hasAnom && !hasStd) continue;

                    sb.AppendLine($"{prefix}   └─ {biome}:");

                    if (hasWater)
                    {
                        string stats = BoosterDiagnostics.AltLowStats_Underwater[hash][biome].GetString();
                        string lineEnd = (hasAnom || hasStd) ? "├─" : "└─";
                        sb.AppendLine($"{prefix}      {lineEnd} Underwater (<0m): {watDict[biome]:N0} {stats}");
                    }
                    if (hasAnom)
                    {
                        string stats = BoosterDiagnostics.AltLowStats_Anomalous[hash][biome].GetString();
                        float floor = BoosterDiagnostics.GetAnomalyFloor(biome);
                        string lineEnd = hasStd ? "├─" : "└─";
                        sb.AppendLine($"{prefix}      {lineEnd} Anomalous (0m to {floor:F0}m): {anoDict[biome]:N0} {stats}");
                    }
                    if (hasStd)
                    {
                        string stats = BoosterDiagnostics.AltLowStats_Standard[hash][biome].GetString();
                        sb.AppendLine($"{prefix}      └─ Standard Failures: {stdDict[biome]:N0} {stats}");
                    }
                }
            }
            ...
            if (BoosterDiagnostics.AltitudeTooHigh.TryGetValue(hash, out var highDict))
            {
                ...
                if (totalHigh > 0)
                {
                    ...
                    {
                        string stats = "";
                        if (BoosterDiagnostics.AltHighStats.TryGetValue(hash, out var statDict) && statDict.TryGetValue(kvp.Key, out var s))
                            stats = s.GetString();
                        sb.AppendLine($"{prefix}   └─ {kvp.Key}: {kvp.Value:N0} {stats}");
                    }
                }
            }
        }
